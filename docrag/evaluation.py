...
# 报告指标的 k；含 ANSWER_PAGES 是因为作答只看前这么多页，coverage@ANSWER_PAGES 直接对应端到端
K = tuple(sorted({1, 3, config.ANSWER_PAGES, 5, 10}))
...
# 一道题的检索指标 --> ({k: recall@k}, {k: 全金标覆盖@k}, 倒数排名)
# recall@k = 金标页落进前 k 名的比例；全金标覆盖@k = 金标页是不是全都落进前 k 名（0 或 1）；
# 倒数排名 = 1 / 第一个金标页的名次，没有记 0
# 例：ranked=['a','b','c','d'], gold_pages={'b','d','x'} --> recall@3 = 1/3，全金标覆盖@3 = 0，倒数排名 = 1/2
# 多金标题上三者会分叉：只找到两页中的一页，recall@4 = 0.5、覆盖 = 0、倒数排名可能是 1.0，
# 而作答模型拿到半份证据基本答不对，所以覆盖才是和端到端对齐的那个指标
def retrieval_metrics(ranked, gold_pages):
    recalls = {k: len(gold_pages & set(ranked[:k])) / len(gold_pages) for k in K}
    coverages = {k: float(gold_pages <= set(ranked[:k])) for k in K}
    ...
    return recalls, coverages, reciprocal_rank
...
def evaluate(retrieval_fn, tag, scope):
    ...
    coverage_sums = {k: 0.0 for k in K}
    ...
    for i, query in enumerate(queries, 1):
        ...
        recalls, coverages, reciprocal_rank = retrieval_metrics(
            ranked, qrels[query["query_id"]]
        )
        for k in K:
            recall_sums[k] += recalls[k]
            coverage_sums[k] += coverages[k]
    ...
    results = {
        ...
        **{f"coverage@{k}": round(coverage_sums[k] / n, 4) for k in K},
...
def save_results(results, run):
    ...
    print("  ".join(f"coverage@{k}={results[f'coverage@{k}']}" for k in K))
