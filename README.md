...
在 [MMLongBench-Doc](https://huggingface.co/datasets/yubo2333/MMLongBench-Doc)（2026-09-14 快照：135 份 PDF、6522 页、1082 题，平均 48 页 / 份）上，闭域检索 **r@1 0.581**、作答 **Acc 0.680**（两条路线按题择一；单用一次作答为 0.650）。所有对比结论都带配对 bootstrap 95% 置信区间。
...
- **两条作答路线，按题择一**：一次作答直接看前 4 页；agent 可以更换关键词重新检索、按页号翻页。两条都跑完后，由一个轻量分类器根据作答过程决定采信哪一条——agent 在跨页题上更强，一次作答在「文档中没有」的题上更可靠。
...
**解析**：PDF 含文本层时直接提取，并保留每个文本块的 bbox；扫描页与文本层乱码的页走 PaddleOCR。页图长边统一缩放到 2000 px，即不影响 OCR 识别结果，同时又降低 OCR 耗时与图像嵌入成本。
...
**路由**：两条路线各有所长，因此不固定用哪一条。两条都跑完后，取 10 个作答过程中可观察的信号——agent 检索了几次、调用了几次模型、看了几页、引用了几页、是否兜底、两边答案长度、两边是否拒答、输出 token 数——交给一个逻辑回归，输出「采信 agent 的概率」，超过阈值用 agent 的答案，否则用一次作答的。权重由 `train_router.py` 离线训练后存进 `docrag/agent/router_weights.json`，代码里只有方程；权重文件不存在时退回「agent 检索过才采信它」的规则，无需任何标注即可运行。
...
| 作答与 agent | qwen3.8-flash | 视觉模型直接读页图；agent 的思考预算限制为 1024 token，一次作答保持默认思考 |
| 追问改写 | qwen3.7-flash | 关闭思考、温度 0、15 秒超时，失败则沿用原问题 |
| 评测抽答案 | qwen3.7-plus | 判分流程中把自由文本答案抽成可比对的形式 |

模型名集中在 `docrag/config.py`，追问改写与评测抽答案的两个常量位于各自模块顶部，更换模型改一行。各环节的选择均有对照实验支撑，见[方法选型](#方法选型)。
...
一次作答是两条作答路线中的一条，即把前 4 页页图直接交给视觉模型：**Acc 0.650 / F1 0.634**。下面的分组用它做基准，另一条路线与路由的结果见[作答路线与路由](#作答路线与路由)。分组 Acc 如下。分组沿用数据集自带判分脚本的口径：单页题按 `evidence_pages` 长度为 1 计，其中含 7 道同样只标了一页的不可回答题，因此三类题数之和略大于总题数；证据类型也允许一题多标。
...
每道题跑两条作答路线再择一。全量 1082 题、同一套检索、逐题配对，指标为 Acc：

| 路线 | 全部（1082） | 单页（501） | 跨页（358） | 不可回答（223） | 每题 |
|---|---|---|---|---|---|
| 一次作答 | 0.650 | 0.712 | 0.488 | 0.771 | ¥0.0130 |
| agent | 0.655 | 0.710 | 0.625 | 0.578 | ¥0.0172 |
| 检索过才采信 agent（无需训练） | 0.669\* | 0.718 | 0.580 | 0.704 | ¥0.0302 |
| **分类器路由** | **0.680\*** | 0.713 | 0.615 | 0.709 | ¥0.0302 |

分类器路由比一次作答高 **+0.030\***（区间 [+0.015, +0.045]）：保住了跨页题的大部分增益（0.615 对 agent 的 0.625），同时把不可回答题从 0.578 拉回 0.709。指标为五折交叉验证的 out-of-fold 值，阈值也在训练折内选定。

**为什么需要路由**：两条路线的强弱是结构性的，不是随机波动。

| 分组 | 一次作答 | agent | 差值 |
|---|---|---|---|
| 跨页（358） | 0.488 | **0.625** | **+0.137\*** |
| 起点证据不全（228） | 0.241 | **0.483** | **+0.241\*** |
| 不可回答（223） | **0.771** | 0.578 | **−0.193\*** |
| 全部（1082） | 0.650 | 0.655 | +0.005 |

单独使用 agent 时总分打平，原因是跨页与检索失败的题上的增益，恰好被不可回答题上的损失抵消——它是一个再检索机制，不是普遍增强。路由的作用就是按题取两者之长。

**泛化验证**：agent 的提示词是在其中 200 题上选定的，因此另报未参与选型的 882 题：跨页 **+0.144\***、起点证据不全 **+0.233\***、不可回答 **−0.209\***、总分 −0.004，与全量一致，说明上述结构不是在选型集上过拟合出来的。路由本身在这 882 题上比一次作答高 **+0.029\***。

**路由学到了什么**：引用页数、查看页数、模型调用次数为正——agent 确实投入了工作才采信它；任一方回答「文档中没有」为负——此时退回一次作答。对生成随机性也更稳：同一批题重新采样一次，裸 agent 由 0.720 降至 0.686，而路由 0.714 → 0.715 基本不变。

**开销**：agent 侧 25% 的题触发检索，平均 1.59 次模型调用，745 题一次作答即结束，兜底强制作答 16 题，工具报错 2 次。路由要求两条路都跑，每题 ¥0.0302，约为一次作答的 2.3 倍；训练成本则很低——实测 20 条标注即可达到 +0.025，40 条进入平台，而且标注要的是「两个回答哪个更好」的成对判断，不需要标准答案。没有权重文件时退回无需标注的手工规则，仍有 **+0.019\***。
...
| **两条路线 + 路由** | **¥0.0302** | 取两者较慢的一条 |
| 重排一次（30 页候选） | ¥0.0096 | — |
| 追问改写 | 不到 ¥0.001 | 约 1 s |

agent 的 p90 反而低于一次作答，因为它单次输出更短；成本高在输入——每多一轮，此前看过的页图要重新发送一次。
...
| agent 限制思考预算 1024 token | 200 题上与默认思考打平，p90 由 68 s 降至 15.6 s；全量作答基线保持默认思考 |
| 不引入 LangChain / LlamaIndex | 框架的抽象层包裹了检索、提示词与 agent 循环，修改细节和排查问题都隔了一层；整条链路自行实现共 2200 行 |
| 两条作答路线按题择一 | 单独使用 agent 总分打平（+0.005）；按作答过程特征路由后 +0.030\*，跨页增益保住 0.615、不可回答回到 0.709 |
